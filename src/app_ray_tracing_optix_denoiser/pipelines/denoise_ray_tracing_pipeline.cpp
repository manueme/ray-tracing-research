...
#include "denoise_ray_tracing_pipeline.h"
#include "../constants.h"
#include <array>
#include <vector>
...
DenoiseRayTracingPipeline::~DenoiseRayTracingPipeline()
{
    ...
    vkDestroyDescriptorSetLayout(m_device, m_descriptorSetLayouts.set5ResultImages, nullptr);
    ...
    uint32_t t_width, uint32_t t_height)
{
    ...
    */
    ...
    std::vector<VkDescriptorSet> rtDescriptorSets = { m_descriptorSets.set0AccelerationStructure,
        ...
        m_descriptorSets.set5ResultImages,
        m_descriptorSets.set6ResultBuffers };
...
void DenoiseRayTracingPipeline::createDescriptorSetsLayout(Scene* t_scene)
{
    ...
    setLayoutBindings.clear();
    setLayoutBindings.push_back(
        // Binding 0 : Result Image Depth Map
    ...
    CHECK_RESULT(vkCreateDescriptorSetLayout(m_device,
        &descriptorLayout,
        nullptr,
        &m_descriptorSetLayouts.set5ResultImages));

    // Set 6: Result buffers
    setLayoutBindings.clear();
    setLayoutBindings = {
        // Binding 0 : Normal Image
        initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
            VK_SHADER_STAGE_RAYGEN_BIT_KHR,
            0),
        // Binding 1 : Albedo image
        initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
            VK_SHADER_STAGE_RAYGEN_BIT_KHR,
            1),
    ...
    CHECK_RESULT(vkCreateDescriptorSetLayout(m_device,
        &descriptorLayout,
        nullptr,
        &m_descriptorSetLayouts.set6ResultBuffers));
...
void DenoiseRayTracingPipeline::createDescriptorSets(VkDescriptorPool t_descriptorPool,
    Scene* t_scene, Buffer* t_sceneBuffer, Buffer* t_instancesBuffer, Buffer* t_lightsBuffer,
    Buffer* t_materialsBuffer)
{
    ...
    VkDescriptorSetAllocateInfo set5AllocInfo
        = initializers::descriptorSetAllocateInfo(t_descriptorPool,
            ...
            1);
    CHECK_RESULT(
        vkAllocateDescriptorSets(m_device, &set5AllocInfo, &m_descriptorSets.set5ResultImages));
    // ----

    // Set 6: Result buffers descriptor
    VkDescriptorSetAllocateInfo set6AllocInfo
        = initializers::descriptorSetAllocateInfo(t_descriptorPool,
            &m_descriptorSetLayouts.set6ResultBuffers,
            1);
    CHECK_RESULT(
        vkAllocateDescriptorSets(m_device, &set6AllocInfo, &m_descriptorSets.set6ResultBuffers));
    ...
    Buffer* t_outImageBuffer)
{
    VkWriteDescriptorSet normalsBufferWrite
        = initializers::writeDescriptorSet(m_descriptorSets.set6ResultBuffers,
            VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
            0,
            &t_normalsBuffer->descriptor);
    VkWriteDescriptorSet albedoBufferWrite
        = initializers::writeDescriptorSet(m_descriptorSets.set6ResultBuffers,
            VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
            1,
    ...
    VkWriteDescriptorSet resultDepthMapWrite
        = initializers::writeDescriptorSet(m_descriptorSets.set5ResultImages,
            VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
            0,
            &t_depthMap->descriptor);
    std::vector<VkWriteDescriptorSet> writeDescriptorSet5 = { resultDepthMapWrite };
    vkUpdateDescriptorSets(m_device,
...
void DenoiseRayTracingPipeline::createShaderBindingTable()
{
